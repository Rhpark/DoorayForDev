---
name: dooray-init
description: 현재 OS와 셸을 확인해 Dooray launcher, PATH와 셸 단축 명령을 최초 설치하거나 복구한다. 사용자가 $dooray-init을 직접 호출했을 때만 사용한다.
---
...
4. 작업공간 밖의 프로필 수정에 승인이 필요하면 요청한다. 기존 내용은 보존한다.
...
7. launcher와 프로필 경로, 추가·갱신 결과를 알린다. PowerShell은 새 창 또는 `. $PROFILE`, bash는 `source ~/.bashrc`, zsh는 `source ~/.zshrc`로 적용한다. Claude/Codex도 다시 시작하도록 안내한다.
