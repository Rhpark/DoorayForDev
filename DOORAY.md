...
`download`는 첨부파일과 **본문 인라인 이미지를 함께** 받는다. Dooray는 본문에 붙여넣은 이미지를
첨부파일 목록 API에 넣지 않으므로, 본문의 HTML `<img src="/files/...">`와 마크다운
`![alt](/files/...)` 양쪽에서 id를 뽑아 내려받는다. 이미지 표기는 이력마다 다르고
`mimeType`(모두 `text/x-markdown`)으로는 구분되지 않는다.
따라서 본문에 이미지가 보이면 "첨부파일 없음" 응답을 근거로 포기하지 않는다.
